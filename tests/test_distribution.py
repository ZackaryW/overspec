...
def wheel_traits(path):
    with zipfile.ZipFile(path) as wheel:
        return {name[len(BUNDLED):]: wheel.read(name)
                for name in wheel.namelist() if name.startswith(BUNDLED)}
...
def test_release_and_sdist_wheels_contain_only_authored_traits(build_tree, tmp_path):
    ...
    for name in ("notes.toml", ".current.toml", ".vars.toml", ".state.json", ".state/trait-hidden.toml"):
        path = profile / name
        path.parent.mkdir(exist_ok=True)
        path.write_text("not a trait")
    for name in ("openspec/config.yaml", "openspec/changes/example/trait-hidden.toml",
                 "openspec/.over/profile-strict/trait-hidden.toml", ".agents/skills/example/SKILL.md"):
        path = build_tree / name
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("not package content")
    expected = {p.relative_to(profile).as_posix(): p.read_bytes()
                for p in profile.rglob("trait*.toml") if ".state" not in p.parts}
    ...
    with zipfile.ZipFile(wheel) as archive:
        assert not any(n.startswith(("openspec/", ".agents/")) for n in archive.namelist())
