...
def test_static_framework_assertion_rejected(project):
    source(
        ...
        declaration(
            ...
            **{
                "assert": {
                    "1": {"assertion": [{"type": "bdd-framework", "name": "behave"}]}
                }
            },
        ),
    )
...
def test_language_or_unrelated_dependencies_do_not_activate(
    project, framework, path, content
):
    identity = framework_source(project, (framework,))
...
def test_flutter_nested_sources(project):
    ...
    (project.root / "pubspec.yaml").write_text(
        "dependencies:\n  flutter:\n    sdk: flutter\n"
    )
