...
def _groups(groups):
    ...
    def visit(name, active):
        ...
        if not isinstance(entries, list):
            raise TypeError(f"dependency-group {name} must be a list")
        names = set()
        for entry in entries:
            if isinstance(entry, str):
                ...
                names.update(
                    visit(
                        canonicalize_name(entry["include-group"], validate=True),
                        (*active, name),
                    )
                )
            else:
...
def detect(evidence):
    ...
    try:
        ...
        if not isinstance(project, dict) or not isinstance(tool, dict):
            raise TypeError("project and tool must be tables")
        if "behave" in tool:
            if not isinstance(tool["behave"], dict):
                raise ValueError("tool.behave must be a table")
            return True
        names = _requirements(
            deep_get(document, ["project", "dependencies"], default=[])
        )
