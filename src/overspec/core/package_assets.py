...
def resource_tree(category, authored):
    ...
    if isinstance(module, Path) and module.name == 'overspec' and module.parent.name == 'src':
        root = module.parent.parent
        project = tomllib.loads(ConfinedPath('pyproject.toml').inspect(root, allowed=(TargetState.FILE,)).target.read_text(encoding='utf-8'))
        if project.get('project', {}).get('name') != 'overspec':
            raise ValueError('Invalid Overspec editable metadata')
        return ConfinedPath(authored).inspect(root, allowed=(TargetState.DIRECTORY,)).target
    tree = module.joinpath('_bundled', *category.split('/'))
    prefix = f'overspec/_bundled/{category}/'
    expected = {str(p)[len(prefix):] for p in metadata.files('overspec') or () if str(p).startswith(prefix)}
    ...
    def visit(directory, prefix=''):
        for item in sorted(directory.iterdir(), key=lambda p: p.name):
            ...
            if isinstance(tree, Path):
                ConfinedPath(relative).inspect(tree, allowed=(TargetState.FILE, TargetState.DIRECTORY))
            if item.is_dir():
                visit(item, relative + '/')
            elif item.is_file():
