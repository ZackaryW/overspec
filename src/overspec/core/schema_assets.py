...
def validate_baseline(baseline):
    if not isinstance(baseline, dict) or set(baseline) != {'name', 'files'} or baseline['name'] != 'overspec':
...
def installed_payload(root):
    plan = ConfinedPath(DESTINATION).inspect(root, allowed=(TargetState.DIRECTORY, TargetState.ABSENT))
...
class SchemaPlan:
    ...
    @property
    def baseline(self):
        return {'name': 'overspec', 'files': {n: fingerprint(b) for n, b in self.desired.items()}}


def plan_schema_install(root, files, baseline):
    validate_baseline({'name': 'overspec', 'files': {n: fingerprint(b) for n, b in files.items()}})
    actual = installed_payload(root)
    for name in files:
        ConfinedPath(DESTINATION + '/' + name).inspect(root, allowed=(TargetState.FILE, TargetState.ABSENT))
    if baseline is None:
        if actual and actual != files:
            raise ValueError('Schema conflict: existing unmanaged tree differs; reconcile or back it up before retrying')
        status = 'adopted-identical' if actual else 'installed'
    else:
        validate_baseline(baseline)
        conflicts = [n for n, h in baseline['files'].items() if n not in actual or fingerprint(actual[n]) != h]
    ...
    return SchemaPlan(root, dict(files), actual,
                      tuple(n for n in files if actual.get(n) != files[n]),
                      tuple(n for n in old if n not in files), status)
...
def install_schema(root):
    ...
    def recheck():
        ...
        if {n: b for n, b in actual.items() if n in relevant} != expected or schema_payload() != files:
    ...
    try:
        ...
        for name in plan.writes:
            storage.atomic_write(root, DESTINATION + '/' + name, files[name], recheck=recheck)
            expected[name] = files[name]
            completed.append(name)
        for name in plan.removals:
            recheck()
            target = ConfinedPath(DESTINATION + '/' + name).inspect(root, allowed=(TargetState.FILE,))
            target.revalidate()
        ...
        raise ValueError(f'Schema publication failed; completed paths: {completed}. '
                         f'Compilation not refreshed. Reconcile schema files and retry update: {exc}') from exc
    return {'status': plan.status if completed or saved is None else 'current',
            'changed': bool(completed), 'path': str(root / DESTINATION),
            'files': completed, 'selection': 'Preserved; select schema: overspec explicitly when desired'}
